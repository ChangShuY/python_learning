...
def loss_net(hidden, label):
    # 第二个隐藏层-->全连接层-->softmax函数-->输出预测值
    prediction = fluid.layers.fc(input=hidden, size=10, act='softmax')
    # 交叉熵代价损失函数
    loss = fluid.layers.cross_entropy(input=prediction, label=label)
    avg_loss = fluid.layers.mean(loss) # 平均值
    acc = fluid.layers.accuracy(input=prediction, label=label) # 准确度
    return prediction, avg_loss, acc

# 多层感知机
def multilayer_perceptron(img, label):
    # 第一个隐藏层：输入层-->全连接层-->tanh函数-->输出img
    img = fluid.layers.fc(input=img, size=200, act='tanh')
    # 第二个隐藏层：输入层为第一个隐藏层-->全连接层-->tanh函数-->输出hidden
...
def convolutional_neural_network(img, label):
    ...
    conv_pool_1 = fluid.nets.simple_img_conv_pool(
        input=img, # img的shape=[样本数n,颜色通道数c,图像高h,图像宽w]
        filter_size=5, # 卷积核大小为5x5
        num_filters=20,# 卷积核的数量
        pool_size=2,   # 池化过滤器大小为2x2
        pool_stride=2, # 池化过滤器步长=2
        act="relu")    # 激活函数
    # 批正则化层
    """
    正则化(regularization)是一种为了减小测试误差的行为，这种行为可能会增加训练误差。
    当我们用神经网络去拟合数据时，很容易出现过拟合现象，也就是训练集表现很好，测试集表现较差。
    这就导致了模型的泛化能力下降。这时候就需要使用正则化，降低模型的复杂度。
    """
...
def train(nn_type,
    ...
    # is_compiled_with_cuda() 返回True表示在 GPU 上运行模型；
    ...
    # paddle.dataset.mnist 加载MNIST数据的模块
    if args.enable_ce:
